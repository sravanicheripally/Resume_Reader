...
def extract_info(data):
    ...
    dic["Clients"] = clients if clients else None
    dic["skills"] = list(skills)
...
# 📝 Removed .doc extraction using win32com.client because it cannot run on Render (Linux)
# If someone uploads a .doc file, handle it gracefully here:

async def extract_text_from_docfile(filedata):
    return [{"error": ".doc files are not supported on this server. Please upload a .docx or PDF file."}]
